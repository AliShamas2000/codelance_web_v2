...
const DeleteNewsletterSubscriptionModal = ({
  isOpen,
  onClose,
  subscription = null,
  onSuccess
  ...
  const handleDelete = async () => {
    ...
    try {
      await newsletterSubscriptionsApi.deleteSubscription(subscription.id)
      ...
      setError(error.response?.data?.message || 'Failed to delete subscription. Please try again.')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  const subscriberEmail = subscription.email || 'this subscription'
      ...
      >
        ...
        <div className="px-6 py-6">
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Are you sure you want to delete the subscription for <span className="font-semibold text-gray-900 dark:text-white">"{subscriberEmail}"</span>? This action cannot be undone.
          </p>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300">
                This will permanently remove the subscription from your newsletter list. All associated data will be deleted.
              </p>
            </div>
          </div>
        </div>
