...
const DeleteContactSubmissionModal = ({
  isOpen,
  onClose,
  submission = null,
  onSuccess
  ...
  const handleDelete = async () => {
    ...
    try {
      await contactSubmissionsApi.deleteContactSubmission(submission.id)
      ...
      setError(error.response?.data?.message || 'Failed to delete submission. Please try again.')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  const submitterName = submission.name || 'this submission'
      ...
      >
        ...
        <div className="px-6 py-6">
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Are you sure you want to delete the submission from <span className="font-semibold text-gray-900 dark:text-white">"{submitterName}"</span>? This action cannot be undone.
          </p>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300">
                This will permanently remove the contact submission from your system. All associated data will be deleted.
              </p>
            </div>
          </div>
        </div>
...
export default DeleteContactSubmissionModal
