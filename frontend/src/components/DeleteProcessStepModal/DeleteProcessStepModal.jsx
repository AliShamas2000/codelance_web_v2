...
const DeleteProcessStepModal = ({
  ...
  const handleDelete = async () => {
    if (!processStep || !processStep.id) {
      setError('Process step information is missing')
      return
    }
    ...
    try {
      await processStepsApi.deleteProcessStep(processStep.id)
      ...
      setError(error.response?.data?.message || 'Failed to delete process step. Please try again.')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  if (!isOpen || !processStep) return null

  const stepTitle = processStep.title || 'this process step'
      ...
      >
        ...
        <div className="px-6 py-6">
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">"{stepTitle}"</span>? This action cannot be undone.
          </p>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300">
                This will permanently remove the process step from your system. All associated data will be lost.
              </p>
            </div>
          </div>
        </div>
...
export default DeleteProcessStepModal
