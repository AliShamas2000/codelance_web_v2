...
const DeleteBannerModal = ({
  ...
  const handleDelete = async () => {
    if (!banner || !banner.id) {
      setError('Banner information is missing')
      return
    }
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Failed to delete banner. Please try again.')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  const bannerTitle = banner.title || banner.name || 'this banner'
      ...
      >
        ...
        <div className="px-6 py-6">
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">"{bannerTitle}"</span>? This action cannot be undone.
          </p>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300">
                This will permanently remove the banner from your system. This action cannot be reversed.
              </p>
            </div>
          </div>
        </div>
